...
public class DBUtilStepDefinitions {
        ...
        public void kullanici_db_util_ile_sutunundaki_verileri_okur(String Email) throws SQLException {
            DBUtils.getResultset().first();

        }
        @Given("DBUtil ile tum {string} degerlerini sira numarasi ile yazdirir")
        public void db_util_ile_tum_degerlerini_sira_numarasi_ile_yazdirir(String Email) throws Exception {
            int satirSayisi=DBUtils.getRowCount();
            DBUtils.getResultset().first();
            int siraNo=1;
            for (int i=1 ; i<satirSayisi ; i++){
                System.out.println(i + ".ci satirdaki price: " + DBUtils.getResultset().getString(Email));
        ...
        public void db_utill_ile_in_oldugunu_test_eder(Integer siraNo, String Email, Integer expectedPrice) throws SQLException {
            DBUtils.getResultset().absolute(siraNo);
            double actualPrice=DBUtils.getResultset().getDouble(Email);
